...
{			
    ...
    double x0 =0.;
    double z0 = nearHit.z();
    x0 = (_mGFtrack->m_axis == TkrCluster::X ? nearHit.x() : nearHit.y());
    KalPar measpar(x0,0.);
				...
				//#########################################################################
{
    ...
    double x0=0;
    double y0=0;
    if (_mGFtrack->m_axis==TkrCluster::X)
    {
    ...
    if (indexhit >= 0) 
    {
	    double x_hit	= (_mGFtrack->m_axis == TkrCluster::X? nearHit.x() : nearHit.y());
	    double x_center = (_mGFtrack->m_axis == TkrCluster::X? centerX.x() : centerX.y());
			 ...
			 //#########################################################################
{
    ...
    if (_mGFtrack->m_axis == TkrCluster::Y)
    {
