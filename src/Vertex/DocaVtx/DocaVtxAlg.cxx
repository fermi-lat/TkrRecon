...
static const AlgFactory<DocaVtxAlg>    Factory;
const IAlgFactory& DocaVtxAlgFactory = Factory;

DocaVtxAlg::DocaVtxAlg(const std::string& name, ISvcLocator* pSvcLoc)
  : Algorithm(name, pSvcLoc)
{
  //Limit on the Doca value:
  declareProperty("docaLimit", m_docaLimit = 10.0/*mm*/ );

}

StatusCode DocaVtxAlg::initialize()
{
  MsgStream log(msgSvc(), name());
  log << MSG::INFO << "initializing DocaVtxAlg" << endreq;
...
StatusCode DocaVtxAlg::execute()
{
  ...
  TkrFitTrackCol* pTracks = 
    SmartDataPtr<TkrFitTrackCol>(eventSvc(),EventModel::TkrRecon::TkrFitTrackCol);
	  ...
	  TkrFitTrack* track2 = *pTrack2++;
            ...
            if (keepIt)
            {
                ...
                if      (arcLen1 > 0. && arcLen2 < 1.)
                {
                    ...
                    if (doca.docaRay1Point2() > m_docaLimit) continue; 
                ...
                else if (arcLen2 > 0. && arcLen1 < 1.)
                {
                    ...
                    if (doca.docaPoint1Ray2() > m_docaLimit) continue; 
...
StatusCode DocaVtxAlg::finalize()
{
  MsgStream log(msgSvc(), name());
  log << MSG::INFO << "finalizing DocaVtxAlg: found " << endreq;
